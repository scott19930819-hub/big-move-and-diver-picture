...
async function createSVG(data) {
    ...
    for (let i = 0; i < actualRows; i++) {
        ...
        // 计算需要增加的高度
        -        // 驱动原因超过2行时，每多一行增加20px
        -        const additionalHeight = driverLineCount > 2 ? (driverLineCount - 2) * 20 : 0;
        -        // 驱动原因超过2行时，每多一行按行高41px增加，保证不溢出
